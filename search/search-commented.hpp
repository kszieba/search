...
public:
	...
	unsigned long expd, gend, reopnd, dups;  //declares all variables for class SearchStats
	...
	void add(SearchStats &other) {  //what is other?
		expd += other.expd;   //are values of these known from functions not defined here?
		gend += other.gend;
		reopnd += other.reopnd;
		dups += other.dups;
...
public:
	...
	bool reached(SearchStats &r) {  //r is?
		return timeup ||
			(expd > 0 && r.expd >= expd) ||
			(gend > 0 && r.gend >= gend);
	}
...
private:
	...
	void timelimit(const char*);  //neither should return anything
};
...
class OpenList {
public:
	const char *kind() {  //constant of character type should be returned
		return "binary heap";
	}

	void push(Node *n) { //takes a pointer to a node called n
		heap.push(n); // node itself should be pushed onto heap
	}

	Node *pop() {  //returns a pointer to a node?
		boost::optional<Node*> p = heap.pop();
		if (!p)
			return NULL;
		return *p; //returns a pointer to p (node popped from the heap)
	}
...
private: //not to be accessed outside this class and friend classes
	struct Heapops {
...
class OpenList <Ops, Node, IntOpenCost> {
	...
	struct Maxq {
		...
		void push(Node *n, unsigned long p) {  //what would p be? (Priority?)
			if (bkts.size() <= p)
				bkts.resize(p+1);
		...
		Node *pop() {
			for ( ; bkts[max].empty() && max > 0; max--) //no initial action
				;
			Node *n = bkts[max].back();
			bkts[max].pop_back();
			Ops::setind(n, -1);
			fill--; //subtracts 1 from fill
			return n;
		}
		...
		bool empty() { return fill == 0; } //function that returns true if fill is 0

		unsigned long fill; //declares fill as unsigned long
		unsigned int max;
		std::vector< std::vector<Node*> > bkts;
	};

	unsigned long fill;  //why is fill found twice? different variables in different scopes?
	unsigned long min;
	std::vector<Maxq> qs;
...
public:
	...
	void push(Node *n) {
		unsigned long p0 = Ops::prio(n); //priority here, tie priority below?
...
void solpath(D &d, Node *goal, Result<D> &res) {  //res is a reference to result
	res.ops.clear();
	res.path.clear();
	for (Node *n = goal; n; n = n->parent) { /*create pointer to the node goal, while node exists,
	node becomes parent pointed to by node*/
		typename D::State buf, &state = d.unpack(buf, n->state);
		res.path.push_back(state);
		if (n->parent)  // if n has pointer to parent
			res.ops.push_back(n->op);  //push back operator preceding it as well
	}
}
