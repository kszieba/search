...
def create_4_rows(inputpath, domain, beam, depth):
    ...
    {"ibs_katrina_full_closed_list": ibs_katrina_full_closed_list, "file": file, "initstate": initstate, "data": data, "beam": beam, 
     "depth": depth}, {})
        wfile.close()
    finalname = "_".join([folder, file, str(beam), str(depth) + "3.csv"])
    ...
    for i in range(len(algorithms)):
        ...
        for line in rfile:
            ...
            if count == 4:
                ...
                break
        if algorithm == "ibs":
            print(",".join([domain, folder, file, str(data), 
                        str(beam), str(depth), rname, g, gcount, excount, gencount,
                        time]), end=", ,")
        else:
            print(",".join([rname, g, gcount, excount, gencount,
                        time]), end=", ,")
        rfile.close()
    print("")
    finalfile.close()
